...
| A long dust storm changes habitat demand in a way that matters to sizing | Not by raising it, which an earlier draft of this submission asserted and which the literature does not support. Crew are always inside a pressurised habitat, so a storm changes no sheltering behaviour. NASA's surface power baseline holds habitat power flat through a storm while rover recharge and drilling go to zero (Cataldo 2009, NASA GRC E-18237, Table II), and puts a crewed field station at about 22 kW of keep-alive against 35 kW of normal operations (Rucker 2016, JSC-CN-35576). What the storm forces is a shed, and the sizing question is the floor left underneath it | No published source found gives a demand uplift figure for a Mars habitat in a dust storm, and the only published direction is downward, so any uplift assumed here is a departure from the NASA baseline and has to be labelled as one. The correlation argument survives either way, because a shed driven by the same weather is still a common cause, but its magnitude is unmeasured. Establishing the non-sheddable floor is the first thing the consortium should ask Tharsis, the habitat modelling partner, to produce |
